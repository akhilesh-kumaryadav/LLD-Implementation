...
public class Pawn extends Piece {
    ...
    public boolean isValidMove(Board board, Cell start, Cell end) {
        ...
        if (colDiff == 0 && !hasMovedBefore(start) && rowDiff == 2) {
            Position intermediate = new Position(start.getPosition().getRow() + direction,
                    start.getPosition().getCol());

            return board.getCell(intermediate).getPiece() == null
                    && board.getCell(end.getPosition()).getPiece() == null;
        }
